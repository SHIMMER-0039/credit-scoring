...
data = pd.read_csv('/home/server02/MLY/dataset/give_me_some_credit_cleaned.csv', low_memory=True)
features = data.drop('SeriousDlqin2yrs', axis=1).replace([-np.inf, np.inf, np.nan], 0)
labels = data['SeriousDlqin2yrs']

print("test3.py file has been successfully loaded!")
        ...
        for learning_rate in param_grid['learning_rate']:
            base_models = [
                DecisionTreeClassifier(
                    criterion='gini',  # 或 'entropy'，根据需要选择
                    max_depth=max_depth,  # 设置最大深度
                    min_samples_split=5,  # 设置节点分裂所需的最小样本数
                    random_state=42
                ),
                ...
                AdaBoostClassifier(
                    base_estimator=DecisionTreeClassifier(max_depth=1),  # 使用弱分类器，通常是决策树桩
                    n_estimators=n_estimators,
