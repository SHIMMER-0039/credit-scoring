...
from main.feature_selection import FeatureEvaluator
...
# Define parameter grid for LightGBM and RandomForest (using integer values)
param_grid = {
    'n_estimators': 400,  # Set as integer, not list
    'max_depth': 5,       # Set as integer
    'learning_rate': 0.2  # Set as float
}
...
for train_proportion in train_proportions:
    ...
    # Train the models using Adaptive Bayesian Stacking with homogeneous models
    base_models = [
        ...
        # xgb.XGBClassifier(eval_metric='logloss', random_state=42, tree_method='hist', device='cuda')
...
results_file_path = os.path.join(save_path, 'shandong_rf_train_proportion_auc_results_with_features.pickle')
