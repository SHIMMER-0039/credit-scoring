...
for dataset_name, cfg in DATASETS.items():
    ...
    data = data.replace([-np.inf, np.inf, np.nan], 0).fillna(0)
    ...
    # -------------------------
    # 4.8 Save single dataset result
    # -------------------------
    result = {
        "dataset_name": dataset_name,
    ...
    dataset_pickle_path = os.path.join(SAVE_DIR, f"AAESS_{dataset_name}_grid_results.pickle")
    with open(dataset_pickle_path, "wb") as f:
        pickle.dump(result, f)

    dataset_json_path = os.path.join(SAVE_DIR, f"AAESS_{dataset_name}_grid_results.json")
    with open(dataset_json_path, "w", encoding="utf-8") as f:
        json.dump(to_serializable(result), f, indent=2, ensure_ascii=False)

    print(f"\nSaved pickle: {dataset_pickle_path}")
    print(f"Saved json:   {dataset_json_path}")

    all_dataset_results[dataset_name] = result


# =========================================================
# 5. Save all dataset results
# =========================================================
all_pickle_path = os.path.join(SAVE_DIR, "AAESS_all_datasets_grid_results.pickle")
with open(all_pickle_path, "wb") as f:
    pickle.dump(all_dataset_results, f)

all_json_path = os.path.join(SAVE_DIR, "AAESS_all_datasets_grid_results.json")
with open(all_json_path, "w", encoding="utf-8") as f:
    json.dump(to_serializable(all_dataset_results), f, indent=2, ensure_ascii=False)

print("\n" + "=" * 100)
print("All datasets finished.")
print(f"Saved all pickle: {all_pickle_path}")
print(f"Saved all json:   {all_json_path}")
print("=" * 100)
